...
public class CommentsDataSource {
	...
	private String[] allColumns = { MySQLiteHelper.COLUMN_ID,
			MySQLiteHelper.COLUMN_COMMENT, MySQLiteHelper.COLUMN_APPROVED };
	...
	public Comment createComment(String comment, boolean approved) {
		...
		if (approved)
		{
			values.put(MySQLiteHelper.COLUMN_APPROVED, 1);
		}
		else
		{
			values.put(MySQLiteHelper.COLUMN_APPROVED, 0);
		}
	...
	private Comment cursorToComment(Cursor cursor) {
		...
		if (cursor.getInt(2) == 0)
		{
			...
			comment.setComment("approved: " + cursor.getString(1));
		}
		else
		{
			comment.setApproved(false);
			comment.setComment("not approved: " + cursor.getString(1));
		}
